...
class Council extends CI_Controller
{
	...
	public function edit_product($product_id = null)
	{
		...
		if ($product_id) {
			...
			$products = $this->CouncilModel->get_all_products();
			$this->load->view('header', $header_data);
			$this->load->view('council_product', array("products" => $products));
			$this->load->view('footer');
		}
	}
	...
	public function edit($product_id)
	{
		...
		$product = $this->db->get_where('products', ['id' => $product_id])->result_array()[0];
		$smes = $this->db->get('smes')->result_array();

		$this->load->view('header', $header_data);
		$this->load->view('council_edit_product', array("product" => $product, "smes" => $smes));
		$this->load->view('footer');
	}
	...
	public function handle_change_sme()
	{
		$previous_sme = $this->input->post("previous_sme");
		$new_sme_name = $this->input->post("new_sme_name");

		if ($new_sme_name == "") {
			$this->session->set_flashdata('error', 'Please choose something');
			redirect($_SERVER['HTTP_REFERER']);
		}

		if ($new_sme_name == $previous_sme) {
			$this->session->set_flashdata('error', 'Nothing to change');
			redirect($_SERVER['HTTP_REFERER']);
		}


		$this->db->where('sme_name', $previous_sme);
		$this->db->update('products', ["sme_name" => $new_sme_name]);
